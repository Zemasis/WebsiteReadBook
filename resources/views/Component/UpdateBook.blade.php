<div class="max-w-7xl w-full mx-auto bg-white shadow-xl rounded-xl overflow-hidden flex">
    ...
    <form action="{{route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="flex w-full">
        @csrf
        @if ($book)
        ...
        <div class="w-1/2 p-6 flex flex-col">
            <h2 class="text-lg font-semibold text-gray-700 mb-4">Cover Book</h2>

            <div class="flex-grow p-2 border border-dashed border-gray-300 rounded-md bg-white flex flex-col items-center ">
                <!-- Preview image -->
                <img id="previewImage" src="{{ asset('storage/' . $book->image) }}" alt="Preview" class="w-80 h-80 object-contain rounded-md" />
        ...
        <!-- Bên phải: Update form -->
        <div class="w-1/2 p-6 flex flex-col">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-semibold text-gray-700">Update Book Information</h2>
            ...
            <div class="space-y-4 flex-grow">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#f1912b]" value="{{ $book->name }}">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="chapter" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#f1912b]" value="{{ $book->chapter }}">
                </div>

                <!-- Description -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <textarea name="description" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#f1912b]" >{{$book->description}}</textarea>
                </div>

                <!-- Review -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Review</label>
                    <input type="text" name="review" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#f1912b]" value="{{ $book->review }}">
                </div>

                <!-- Tags -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tags</label>
                    <input type="text" name="tag" placeholder="Type to search..." class="w-full px-4 py-2 border border-gray-300 rounded-md" value="{{ $book->tag }}">
                </div>
            </div>
            ...
            <div class="flex justify-between items-center mt-6">
                ...
                <button type="submit" class="px-4 py-2 bg-[#f1912b] text-white font-semibold rounded-md hover:bg-[#e47a18]">Update</button>
            </div>
